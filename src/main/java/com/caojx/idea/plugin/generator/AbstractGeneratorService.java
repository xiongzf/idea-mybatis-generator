...
public abstract class AbstractGeneratorService implements IGeneratorService {
    ...
    public Map<String, Object> getObjectMap(GeneratorContext generatorContext, TableInfo tableInfo) {
        ...
        // 表名转基础驼峰
        String baseEntityName = CaseFormat.UPPER_UNDERSCORE.to(CaseFormat.UPPER_CAMEL, tableInfo.getName().replace(generatorContext.getTablePrefix(), ""));
        ...
        if (StringUtils.isNotBlank(controllerProperties.getNamePrefix())) {
            controllerName = controllerProperties.getNamePrefix() + controllerName;
        }
